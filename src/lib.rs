...
pub struct State {
    // record request number of this contract.
    request_num : u32
}
...
#[init(contract = "hackaton_concordium_task_2_record_contract")]
fn init<S: HasStateApi>(
    _ctx: &impl HasInitContext,
    _state_builder: &mut StateBuilder<S>,
) -> InitResult<State> {
    // Your code
    let i_state = State {request_num: 0};
    Ok(i_state)
}
...
#[receive(
    contract = "hackaton_concordium_task_2_record_contract",
    name = "receive",
    parameter = "bool",
    error = "Error",
    mutable
...
#[receive(contract = "hackaton_concordium_task_2_record_contract", name = "view", return_value = "State")]
